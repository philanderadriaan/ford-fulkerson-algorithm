...
public class RandomGraph
{
  ...
  public static void main(String[] args)
  {
    ...
    else if (Integer.parseInt(args[0]) > Integer.parseInt(args[1]))
    {
      if (Integer.parseInt(args[3]) >= Integer.parseInt(args[2]))
      {
        ...
        {
          toFile(sparseGraphBuilder(Integer.parseInt(args[0]), Integer.parseInt(args[2]),
                                    Integer.parseInt(args[3])), args[4]);
        }
        else
        {
          toFile(graphBuilder(Integer.parseInt(args[0]), Integer.parseInt(args[1]),
    ...
    else
    {
      System.out.println("\nFAIL!");
      System.out
          .println("The number of vertices must exceed the number of edges leaving each node.");
    }
  }
   ...
   * @return StringBuffer object of the graph.
   */
  public static StringBuffer sparseGraphBuilder(int v, int min, int max)
  {
    int c;
    ...
    bfr.append("s" + " " + "v" + 1 + " " + c + NL);

    for (int i = 1; i < v; i++)
    {
      c = min + gen.nextInt(max - min + 1);
      bfr.append("v" + i + " " + "v" + (i + 1) + " " + c + NL);
    }

    c = min + gen.nextInt(max - min + 1);
    bfr.append("v" + v + " " + "t" + " " + c + NL);
